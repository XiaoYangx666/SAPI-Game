// One-way: marker.ts declares no dependencies, so this cannot cycle.
import type { TraceErrorPayload } from "./marker";
...
/**
 * Event ids are the wire format. A const object rather than a TypeScript `enum`
 * on purpose: an enum emits a reverse-mapping bootstrap that nothing uses, and
 * `@begame/core` bundles this module purely for the forward values.
 */
...
/**
 * Branded reference to a player inside a trace payload.
 *
 * The brand symbol lives here, rather than in the session implementation, so the
 * runtime and the codec recognize the same marker without sharing a module.
 */
export const playerMarker = Symbol("begame.trace.player");

export interface TracePlayerMarker {
 ...
 *
 * A deferred error marker is a legal payload value: it is replaced by the
 * serialized error during normalization and never reaches the wire.
 */
