...
test("agent API exposes capabilities and a structured analysis", async () => {
    ...
    expect(healthBody.capabilities).toEqual({ http: true, connect: false, net: false, ingest: false });
    ...
    const analyzed = await app.request("/api/analyze", { method: "POST", body: realTrace() });
    ...
    expect(analysisBody.analysis.domainCount).toBeGreaterThan(100);
    ...
    const decoded = await app.request("/api/decode", { method: "POST", body: realTrace() });
    const decodedBody = await decoded.json();
    expect(decodedBody.ok).toBe(true);
    expect(decodedBody.analysis.gameType).toBe("doudizhu");
    expect(decodedBody.selected.events.length).toBe(340);
});
