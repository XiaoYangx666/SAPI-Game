...
-   正在挂起的 wait() 会在 cancel() 调用时被立刻 reject，不必等延迟 tick 走完；
-   wait() 正常结束后还会再检查一次，覆盖"等待期间被取消"的竞态。

> ps: 在大多数情况下，你只需要使用 wait()，它会在进入时、结束时、以及被 cancel() 时三处保证取消能生效。
...
-   运行系统的 runJob
    ...
    调用 cancel()（或 RunnerManager.dispose()）时，Job 被清除，这个 Promise 也会被立即 resolve，不会永久挂起。
